...
// POST /api/asesor/visits/[id]/checkout - Complete a visit
export async function POST(
  ...
  try {
    ...
    const { advisor_notes, latitude, longitude } = body
    ...
    if (advisor_notes) {
      ...
      const existingNotes = currentVisit?.advisor_notes || ''
      updateData.advisor_notes = existingNotes
        ? `${existingNotes}\n\n[Checkout] ${advisor_notes}`
        : `[Checkout] ${advisor_notes}`
    }
    ...
    console.error('Error en POST /api/asesor/visits/[id]/checkout:', error)
