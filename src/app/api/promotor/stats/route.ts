...
export async function GET() {
  try {
    ...
    // 2. Obtener user_profile del promotor
    ...
    // 4. Validar que tenga rol de promotor activo
    ...
    if (!promotorRole) {
      return NextResponse.json(
        { error: 'Usuario no tiene rol de promotor activo' },
        { status: 403 }
      )
    }

    const promotorId = userProfile.id
    // brandId y tenantId disponibles para futuras funcionalidades
    // const brandId = promotorRole.brand_id
    // const tenantId = promotorRole.tenant_id
    ...
    console.error('Error en /api/promotor/stats:', error)
