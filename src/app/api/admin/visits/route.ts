...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build visits query — RLS handles tenant scoping
    let query = supabase
      .from('visits')
      .select(`
        id,
    ...
    if (visitsError) {
      console.error('Error fetching admin visits:', visitsError)
    ...
    const transformedVisits = (visits || []).map((visit) => {
      ...
      const brand = v.brand as Record<string, unknown> | null
      ...
      return {
        ...
        client_id: client?.id || v.client_id,
        client_name: (client?.business_name as string) || 'Cliente desconocido',
        brand_name: (brand?.name as string) || 'Sin marca',
    ...
    const filtered = search
      ? transformedVisits.filter(v =>
          ...
          v.promotor_name.toLowerCase().includes(search.toLowerCase())
        )
      : transformedVisits
    ...
    console.error('Error in GET /api/admin/visits:', error)
