...
export default function CreateZonePage() {
  ...
  const [formData, setFormData] = useState<ZoneCreateForm>({
    name: '',
    code: '',
    zone_type: '',
    description: '',
  ...
  const loadInitialData = async () => {
    ...
    try {
      // Load all zones for parent dropdown (no pagination, all active)
      const response = await fetch('/api/admin/zones?limit=200&is_active=true');
      const result = await response.json();
      if (response.ok) {
        setParentZones(result.data || []);
      }
    } catch (err) {
  ...
  const handleInputChange = (field: keyof ZoneCreateForm, value: string | number | boolean) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/zones/create', {
        method: 'POST',
      ...
      <div className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Crear Zona</h1>
