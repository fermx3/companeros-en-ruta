...
import {
  isDuplicateStaffSurveyResponse,
  useStaffSurvey,
  useSubmitStaffSurvey,
  type StaffSurveyQuestion,
} from '@/features/staff-surveys/api'

// Survey input chips/options use StyleSheet for the Pressable+map crash
// workaround documented in Button.tsx. Screen-local because the radio /
// checkbox visuals don't fit the broader SegmentedControl primitive.

type Answers = Record<string, unknown>

export function SurveyRunnerScreen() {
  const { surveyId } = useLocalSearchParams<{ surveyId: string }>()
  const surveyQuery = useStaffSurvey(surveyId)
  const submit = useSubmitStaffSurvey(surveyId!)
  ...
  async function onSubmit() {
    ...
    try {
      ...
      if (isDuplicateStaffSurveyResponse(e)) {
        Alert.alert('Ya respondiste', 'Ya enviaste tu respuesta a esta encuesta. ¡Gracias!', [
          { text: 'Volver', onPress: () => router.back() },
        ])
        return
      }
...
function QuestionRow({ index, question, value, onChange }: QuestionRowProps) {
  return (
    <Card className="mb-2">
      ...
      <Text className="text-sm font-bold text-navy mb-2">{question.question_text}</Text>
