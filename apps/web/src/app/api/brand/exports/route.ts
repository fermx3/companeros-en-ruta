...
async function resolveFilteredClientIds(
  ...
  if (filters.client_status?.length) {
    clientIds = clientIds.filter(id => {
      ...
      return c && c.status != null && filters.client_status!.includes(c.status)
    })
  }
  if (filters.client_type_ids?.length) {
    clientIds = clientIds.filter(id => {
      const c = clientMap.get(id)
      return c && c.client_type_id != null && filters.client_type_ids!.includes(c.client_type_id)
    })
  }
  if (filters.market_ids?.length) {
    clientIds = clientIds.filter(id => {
      const c = clientMap.get(id)
      return c && c.market_id != null && filters.market_ids!.includes(c.market_id)
    })
  }
  if (filters.commercial_structure_ids?.length) {
    clientIds = clientIds.filter(id => {
      const c = clientMap.get(id)
      return c && c.commercial_structure_id != null && filters.commercial_structure_ids!.includes(c.commercial_structure_id)
    })
  }
  if (filters.zone_ids?.length) {
    clientIds = clientIds.filter(id => {
      const c = clientMap.get(id)
      return c && c.zone_id != null && filters.zone_ids!.includes(c.zone_id)
    })
  }
  if (filters.states?.length) {
    clientIds = clientIds.filter(id => {
      const c = clientMap.get(id)
      return c && c.address_state != null && filters.states!.includes(c.address_state)
    })
  }
  if (filters.cities?.length) {
    clientIds = clientIds.filter(id => {
      const c = clientMap.get(id)
      return c && c.address_city != null && filters.cities!.includes(c.address_city)
    })
  }
  if (filters.postal_codes?.length) {
    clientIds = clientIds.filter(id => {
      const c = clientMap.get(id)
      return c && c.address_postal_code != null && filters.postal_codes!.includes(c.address_postal_code)
    })
  }
  if (filters.registration_date_from) {
    clientIds = clientIds.filter(id => {
      const c = clientMap.get(id)
      return c && c.registration_date != null && c.registration_date >= filters.registration_date_from!
    })
  }
  if (filters.registration_date_to) {
    clientIds = clientIds.filter(id => {
      const c = clientMap.get(id)
      return c && c.registration_date != null && c.registration_date <= filters.registration_date_to!
    })
  }
