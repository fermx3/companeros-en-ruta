-- ============================================================================
-- Migration: Fix validate functions to use promotor_id instead of advisor_id
-- ============================================================================
-- The validate_visit_data() and validate_visit_order_data() functions still
-- reference the old advisor_id column which was renamed to promotor_id
-- ============================================================================

BEGIN;

-- ============================================================================
-- Update validate_visit_data() function
-- ============================================================================
...
-- ============================================================================
-- Update validate_visit_order_data() function
-- ============================================================================
...
declare
  ...
  promotor_has_valid_role boolean := false;
begin
  -- Validar que promotor_id tenga rol válido y pertenezca al tenant
  select exists (
    select 1 from public.user_roles ur
    where ur.user_profile_id = new.promotor_id
    and ur.role in ('promotor', 'supervisor', 'asesor_de_ventas')
    and ur.status = 'active'
    and ur.deleted_at is null
  ) into promotor_has_valid_role;
  ...
  if promotor_tenant_id is null then
    raise exception 'Usuario no tiene roles activos';
  end if;

  if promotor_tenant_id != new.tenant_id then
    raise exception 'El usuario no pertenece al tenant especificado';
  end if;
